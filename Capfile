...
set :nhosts, 1
set :group_name, 'FloResCre_ChIPseq'
...
task :remove_zips, :roles => group_name do
   files = capture("ls #{mount_point}/*.zip")
   files = files.split("\n")
   files.each {|f| ("rm '#{f}'") }
end
before 'remove_zips', 'EC2:start'
...
task :s3_config, :roles => group_name do
  upload("/home/cassj/.s3cfg", '/home/ubuntu/.s3cfg')
end 
...
task :run_bowtie, :roles => group_name do
  ...
  files.each{|infile|
    ...
    run("#{working_dir}/bowtie-0.12.7/bowtie --sam -k1 -l15 -n1 -m3 -p20 --solexa1.3-quals -q mm9 #{infile} --quiet > #{outfile} ")
  } 
